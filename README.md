...
- 🔢 **ペア番号採番**: P写真・M写真に共通番号を自動割り当て (1,1,2,2,3,3...)
- ✏️ **手動番号編集**: 自動番号を手動で変更可能
...
- **画像プレビュー**: 最大800x600pxでの画像表示（アスペクト比維持）、自然順序ソート対応
- **ファイル名規則**: `番号_部品名_重量_単位_素材ID_加工ID_写真区分_特記事項.拡張子`
- **ペア番号システム**: P写真とM写真に同じ番号を割り当て（1,1,2,2,3,3...形式）
...
1. 左側の入力パネルに各項目を入力（スクロール可能）
   - **番号**: ペア番号（1,1,2,2,3,3...）で自動設定、手動編集可能
...
### ペア番号システムの例
P写真とM写真に同じ番号が割り当てられます：

```
1_ねじ_5_g_M001_K001_P_0.jpg      # 1枚目：部品のみ
1_ねじ_5_g_M001_K001_M_0.jpg      # 2枚目：素材込み
2_ボルト_10_g_M002_K002_P_0.jpg   # 3枚目：部品のみ
2_ボルト_10_g_M002_K002_M_0.jpg   # 4枚目：素材込み
3_ワッシャー_2_g_M003_K001_P_1.jpg # 5枚目：部品のみ（特記事項あり）
3_ワッシャー_2_g_M003_K001_M_1.jpg # 6枚目：素材込み（特記事項あり）
```
...
- **重量**: 数値
- **単位**: kg または g
- **素材ID**: ExcelマスターのID（例：M001）
- **加工ID**: ExcelマスターのID（例：K001）
- **写真区分**: P（部品のみ）または M（素材込み）
- **特記事項**: 0（なし）または 1（あり）
...
### v2.1.0 (2025-10-30)
- ペア番号採番システム実装（1,1,2,2,3,3...形式）
- 手動番号入力フィールド追加（自動番号を編集可能）
- 自然順序ソート機能実装（1,2,3...9,10,11の正しい順序）
- P写真・M写真の番号統一機能
